describe('Search block e2e Test', function () {
  ...
  afterEach(function () {
    ...
    let moveToTrashDialogPath = '//div[@class="modal-dialog"][contains(.,"This note will be moved to trash")]';
    waitVisibility(element(by.xpath(moveToTrashDialogPath)));
    let okButton = element(by.xpath(moveToTrashDialogPath + '//div[@class="modal-footer"]//button[contains(.,"OK")]'));
    clickOn(okButton);
  });
  ...
  let getReplaceInput = function () {
    return element(by.xpath('//div[contains(@class, "search-group")]//span[text()="Replace"]//..//input'));
  };

  let getNextOccurrenceButton = function () {
    return element(by.xpath('//div[contains(@class, "search-group")]' + '/div/button[@ng-click="nextOccurrence()"]'));
  };

  let getPrevOccurrenceButton = function () {
    return element(by.xpath('//div[contains(@class, "search-group")]' + '/div/button[@ng-click="prevOccurrence()"]'));
  };
  ...
  let getReplaceAllButton = function () {
    return element(by.xpath('//div[contains(@class, "search-group")]//button[@ng-click="replaceAll()"]'));
  };

  let getMatchesElement = function () {
    return element(by.xpath('//div[contains(@class, "search-group")]' + '//span[contains(@class, "after-input")]'));
  };
  ...
  let openSearchBoxByShortcut = function () {
    browser
      .switchTo()
      .activeElement()
      .sendKeys(protractor.Key.chord(protractor.Key.CONTROL, protractor.Key.ALT, 'f'));
  };
  ...
  let checkFind = function (findInput, text, expectedMatchesCount) {
    ...
    let matchesCount = element(
      by.xpath('//div[contains(@class, "search-group")]' + '//span[contains(@class, "after-input")]'),
    );
    matchesCount.getText().then(function (text) {
      expect(text.indexOf((expectedMatchesCount === 0 ? 0 : 1) + ' of ' + expectedMatchesCount) !== -1).toBe(true);
    });
  };
  ...
  it('correct count of selections', function () {
    ...
    var markers = element.all(
      by.xpath(
        '//code-editor/div/div[@class="ace_scroller"]' +
          '/div[@class="ace_content"]/div[@class="ace_layer ace_marker-layer"]/div',
      ),
    );
    expect(markers.count()).toEqual(
      countSubstringOccurrence(testData.textInFirstP, subs) +
  ...
  it('correct matches count number', function () {
    ...
    checkFind(
      findInput,
      subs,
      countSubstringOccurrence(testData.textInFirstP, subs) + countSubstringOccurrence(testData.textInSecondP, subs),
    );
    subs = 'te';
    checkFind(
      findInput,
  ...
  it('matches count changes correctly after replace', function () {
    ...
    let matchesElement = getMatchesElement();
    let matchesCount =
      countSubstringOccurrence(testData.textInFirstP, textToFind) +
