...
function NavCtrl(
  ...
  function getZeppelinVersion() {
    $http
      .get(baseUrlSrv.getRestApiBase() + '/version')
  ...
  function getClusterAddr() {
    $http
      .get(baseUrlSrv.getRestApiBase() + '/cluster/address')
      .success(function (data, status, headers, config) {
      ...
      function (response) {
        ...
        if (clearAuthorizationHeader === 'true') {
          if (detectIE()) {
            ...
            if (!outcome) {
              let m =
                'Your browser is too old or too weird to support log out functionality. Close all windows and ' +
                'restart the browser.';
              alert(m);
            }
          } else {
